...
public class ReportAction extends ActionBase {
     ...
     * いいね一覧を表示する
     */
    public void favoriteShow() throws ServletException, IOException{

    	//IDを条件に日報データを取得する
    	ReportView rv = service.findOne(toNumber(getRequestParam(AttributeConst.REP_ID)));
    	//指定した日報についたいいねを、指定されたページ数の画面一覧に表示する分取得する
    	int page = getPage();
    	...
    	putRequestScope(AttributeConst.FAV_COUNT, favoritecount); //指定した日報についたいいねの件数
    	putRequestScope(AttributeConst.PAGE, page); //ページ数
    	putRequestScope(AttributeConst.MAX_ROW, JpaConst.ROW_PER_PAGE);//1ページに表示する件数
